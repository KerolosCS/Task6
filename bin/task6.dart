...
//task2
  ...
  // List namesShare = [];
  post1Likes.forEach((key, value) {
    emailsLike.add(post1Likes[key]['info']['email']);
  });
  ...
  post1Share.forEach((key, value) {
    emailsLike.forEach((element) {
      if (element == post1Share[key]['email']) {
        likeAndSharePost1.add(key);
      }
    });
  });
...
//task5
  ...
  if (oldUser1[old1key] > oldUser2[old2key]) {
    ...
    // sentanceDraw('kerolos fady ezzat');
  }
  // print(maxAge);
}
